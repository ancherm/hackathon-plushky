...
import SSTU from "../../assets/images/sstu.svg";
...
class Navbar extends Component {
    ...
    getErrorMessage(prevProps) {
        if (this.props.fetchAllUnreadMessages.hasError && prevProps.fetchAllUnreadMessages.error !== this.props.fetchAllUnreadMessages.error) {
            return this.props.fetchAllUnreadMessages.message || 'Ошибка сервера';
        }
        else if (this.props.searchResultsData.hasError && prevProps.searchResultsData.error !== this.props.searchResultsData.error) {
            return this.props.searchResultsData.message || 'Ошибка сервера';
        }
                        ...
                        <div className="navbar-wrapper">
                            <div className="nav-searchbar-container">
                                <div className="site-logo">
                                    <NavLink to="/" className="nav-link " ><img src={SSTU} alt="SSTU"/></NavLink>
                                </div>
                                {loggedIn && <form className="form-inline my-2 my-lg-0" onSubmit={this.searchFriend}>
                                    ...
                                    <button className="btn button-navbar-outline my-2 my-sm-0" type="submit">Поиск</button>
                                </form>}
                            </div>

                            <label id="toggle" htmlFor="main-nav-toggle" style={{ 'marginBottom': '0' }}><span>Меню</span></label>

                            <nav className="nav-main">
                                <ul className="nav-ul">
                                    {loggedIn && <li className="nav-item"><NavLink exact to={`/home/profile/${userId}`} className="nav-link  fas fa-user tooltipCustom"  > {userService.getUsername()}<span className="tooltiptextCustom">Профиль</span></NavLink></li>}

                                    {loggedIn && <li className="nav-item"><NavLink exact to={`/home/comments/${userId}`} className="nav-link ">Домой</NavLink></li>}
                                    {loggedIn && <li className="nav-item"><NavLink exact to={`/home/findFriends/${userId}`} className="nav-link " >Найти друзей</NavLink></li>}
                                    ...
                                    {loggedIn &&
                                        <li className="nav-item">
                                            <NavLink exact to={`/home/friendRequests/${userId}`} className="nav-link tooltipCustom">
                                                ...
                                                <span className="tooltiptextCustom" id="friend-requests-tooltip">Запросы дружбы</span>
                                            </NavLink>
                                        </li>}
                                        ...
                                        >
                                            <div className="icon-badge-wrapper">
                                                <NavLink className="fas fa-envelope tooltipCustom nav-link" to="#">
                                                    <span className="tooltiptextCustom">Сообщения</span>
                                                </NavLink>
                                    ...
                                    {loggedIn && <li className="nav-item"><NavLink exact to="#" className="nav-link " onClick={onLogout} >Выйти</NavLink></li>}
                                    {!loggedIn && <li className="nav-item"><NavLink exact to="/login" className="nav-link" >Войти</NavLink></li>}
                                    {/*{!loggedIn && <li className="nav-item"><NavLink exact to="/register" className="nav-link" >Регистрация</NavLink></li>}*/}
                                </ul>
                            </nav>
                        </div>
                    </section>
