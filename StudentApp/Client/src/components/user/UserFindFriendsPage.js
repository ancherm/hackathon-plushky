...
class UserFindFriendsPage extends Component {
    ...
    render() {
        ...
        if (requestLength > 0) {
            requests = (
                <Fragment>
                    <h3 className="mt-5">Отвечайте на Ваши запросы о добавлении в друзья</h3>
                    ...
                    {this.props.userWaitingForAcceptingRequest.map((friend) =>
                        <FriendRequest
                            ...
                            thirdButtonText={'Посмотреть профиль'}
        ...
        if (waitingForResponseUsers > 0) {
            friendsCandidates = (
                <Fragment>
                    <h3 className="mt-5">Ожидающие рассмотрения запросы</h3>
                    <div className="hr-styles"></div>
                    {
                        this.props.usersReceivedRequestFromCurrentUser.map((friend) =>
                            <Friend
                                ...
                                secondButtonLink={`/`}
                                firstButtonText={'Посмотреть профиль'}
                                secondButtonText={'Отклонить запрос'}
                                secondButtonOnClick={this.rejectRequest}
                            />)
                    }
                </Fragment>
        ...
        if (friendsCandidatesArr > 0) {
            remainCandidates = (
                <Fragment>
                    <h3 className="mt-5">Люди, которых Вы, Возможно, знаете</h3>
                    <div className="hr-styles"></div>
                    {
                        this.props.friendsCandidatesArr.map((friend) =>
                            <Friend
                                ...
                                secondButtonLink={`/`}
                                firstButtonText={'Посмотреть профиль'}
                                secondButtonText={'Добавить одногруппника'}
        ...
        if (!requests && !friendsCandidates && !remainCandidates) {
            noResult = (
                <Fragment>
                    <h2>Все зарегистрированные пользователи уже являются вашими друзьями!</h2>
                    <div className="hr-styles"></div>
                </Fragment>
            )
        }
