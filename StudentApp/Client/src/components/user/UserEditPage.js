...
class UserEditPage extends Component {
    ...
    getErrorMessage(prevProps) {
        if (this.props.updateUserData.hasError && prevProps.updateUserData.error !== this.props.updateUserData.error) {
            return this.props.updateUserData.message || 'Ошибка сервера';
        }
        return null;
    }
                ...
                <article className="main-article-shared-content">
                    <section className="form-content-section">
                        <div className="container mb-4">
                            <h1 className="text-center font-weight-bold mt-4" style={{ 'margin': '1rem auto' }}>Изменить аккаунт</h1>
                            ...
                            <form className="Register-form-container  " onSubmit={this.onSubmitHandler} >
                                ...
                                {shouldMarkError('username') && <small id="usernameHelp" className="form-text alert alert-danger"> {(!this.state.username ? 'Username is required!' : 'Username should be at least 4 and maximum 16 characters long!')}</small>}
                                        ...
                                        <div className="form-group">
                                            <label htmlFor="firstName" className="font-weight-bold" >Имя</label>
                                            <input
                                                type="text"
                                        ...
                                        <div className="form-group">
                                            <label htmlFor="address" className="font-weight-bold" >Адрес</label>
                                            <input
                                                type="text"
                                        ...
                                        {showPicsButtons && <div className="form-group">
                                            <label htmlFor="profilePicUrl" className="font-weight-bold" >URL-адрес изображения профиля!</label>
                                            <input
                                                type="text"
                                                ...
                                                placeholder="Введите URL-адрес изображения профиля!"
                                            />
                                            {shouldMarkError('profilePicUrl') && <small id="profilePicUrl" className="form-text alert alert-danger">{(!this.state.profilePicUrl ? 'Требуется URL-адрес изображения профиля!' : '')}</small>}
                                        </div>}
                                ...
                                {shouldMarkError('email') && <small id="emailHelp" className="form-text alert alert-danger">{(!this.state.email ? 'Email is required!' : 'Invalid e-mail address!')}</small>}
                                        ...
                                        <div className="form-group">
                                            <label htmlFor="lastName" className="font-weight-bold">Фамилия</label>
                                            <input
                                                type="text"
                                        ...
                                        <div className="form-group">
                                            ...
                                            />
                                            {shouldMarkError('city') && <small id="cityHelp" className="form-text alert alert-danger">{(!this.state.city ? 'Город обязателен' : '')}</small>}
                                        </div>

                                        {showPicsButtons && <div className="form-group">
                                            <label htmlFor="backgroundImageUrl" className="font-weight-bold" >URL-адрес изображения обложки</label>
                                            <input
                                                type="text"
                                                ...
                                                placeholder="Введите URL-адрес изображения обложки"
                                            />
                                            {shouldMarkError('backgroundImageUrl') && <small id="backgroundImageUrlHelp" className="form-text alert alert-danger">{(!this.state.backgroundImageUrl ? 'Требуется URL-адрес изображения на обложке!' : '')}</small>}
                                        </div>}
                                    </section>
                                </div>
                                ...
                                <div className="text-center">
                                    <button disabled={!isEnabled} type="submit" className="btn App-button-primary btn-lg m-3">Изменить</button>
                                    <NavLink className="btn App-button-primary btn-lg m-3" to={`/home/profile/${this.props.id}`} role="button">Отмена</NavLink>
