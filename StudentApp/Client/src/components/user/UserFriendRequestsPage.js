...
class UserFriendRequestsPage extends Component {
    ...
    render() {
        ...
        if (requestLength > 0) {
            requests = (
                <Fragment>
                    <h3 className="mt-5">Отвечайте на Ваши запросы о добавлении в одногруппники</h3>
                    ...
                    {this.props.userWaitingForAcceptingRequest.map((friend) =>
                        <FriendRequest
                            ...
                            thirdButtonText={'Посмотрет профиль'}
        ...
        if (!requests) {
            requests = (
                <Fragment>
                    <h2>Здесь нет запросов в одногруппники!</h2>
                    <div className="hr-styles"></div>
                </Fragment>)
        }
