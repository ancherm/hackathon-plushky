...
class UserGalleryPage extends Component {
    ...
    uploadFile = () => {
        ...
        if (!this.state.file) {
            toast.error(<ToastComponent.errorToast text='Пожалуйста, загрузите файл.' />, {
                position: toast.POSITION.TOP_RIGHT
            });
            return;
        }
        ...
        if (this.state.file.size >= 2000000) {
            toast.error(<ToastComponent.errorToast text='Размер файла превышает установленный лимит в 2 МБ.' />, {
                position: toast.POSITION.TOP_RIGHT
            });
            return;
        }
                ...
                <article className="aside-article-photos">
                    <div className="gallery-article-intro">
                        ...
                        <div className="gallery-aside-article-header"  >
                            ...
                            <h3 className="aside-article-title">Документы</h3>
                        </div>

                        {(isRoot || isTheCurrentLoggedInUser) && <div className="">
                            <button className="button update-info">
                                <label className="cursor-pointer" id="upload" htmlFor="fileUpload" > Добавить документ
                    ...
                    {this.props.picturesArr.length > 0
                        ...
                        <Fragment>
                            {!this.state.ready ?
                                ...
                                <Fragment>
                                    <div className="hr-styles" style={{ 'width': '90%' }}></div>
                                    <h3 className="text-center">Документы отсутствуют у <span className="username-gallery">{`${formattedUserNames}`}</span> !</h3>
                                    <div className="hr-styles" style={{ 'width': '90%' }}></div>
                                </Fragment>
                            }
                        </Fragment>
