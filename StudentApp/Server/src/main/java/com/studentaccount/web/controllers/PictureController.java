...
import com.studentaccount.domain.models.viewModels.picture.PictureAllViewModel;
...
public class PictureController {
    ...
    public ResponseEntity<Object> addPicture(
        ...
        if (result) {
            SuccessResponse successResponse = new SuccessResponse(
                    LocalDateTime.now(),
                    ResponseMessageConstants.SUCCESSFUL_PICTURE_UPLOAD_MESSAGE,
                    "",
                    true
            );
    ...
    public ResponseEntity<Object> removePicture(@RequestBody Map<String, Object> body) throws Exception {
        ...
        if (result) {
            SuccessResponse successResponse = new SuccessResponse(
                    LocalDateTime.now(),
                    ResponseMessageConstants.SUCCESSFUL_PICTURE_DELETE_MESSAGE,
                    "",
                    true
            );
            return new ResponseEntity<>(this.objectMapper.writeValueAsString(successResponse), HttpStatus.OK);
