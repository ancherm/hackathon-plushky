...
import com.studentaccount.domain.models.viewModels.user.UserAllViewModel;
...
import com.studentaccount.utils.responseHandler.exceptions.BadRequestException;
...
public class UserController {
    ...
    public ResponseEntity<Object> registerUser(@RequestBody @Valid UserRegisterBindingModel userRegisterBindingModel) throws Exception {
        ...
        if (!userValidationService.isValid(userRegisterBindingModel.getPassword(),
        ...
        SuccessResponse successResponse = successResponseBuilder(
                LocalDateTime.now(),
    ...
    public ResponseEntity updateUser(@RequestBody @Valid UserUpdateBindingModel userUpdateBindingModel,
        ...
        if (result) {
            SuccessResponse successResponse = successResponseBuilder(
                    LocalDateTime.now(),
                    ResponseMessageConstants.SUCCESSFUL_USER_PROFILE_EDIT_MESSAGE,
                    "",
                    true
            );
            return new ResponseEntity<>(this.objectMapper.writeValueAsString(successResponse), HttpStatus.OK);
    ...
    public ResponseEntity<Object> deleteUser(@PathVariable String id) throws Exception {
            ...
            if(result){
                SuccessResponse successResponse = successResponseBuilder(
                        LocalDateTime.now(),
                        ResponseMessageConstants.SUCCESSFUL_USER_DELETE_MESSAGE,
                        "",
                        true
                );
                return new ResponseEntity<>(this.objectMapper.writeValueAsString(successResponse), HttpStatus.OK);
    ...
    public ResponseEntity promoteUser(@RequestParam(name = "id") String id) throws Exception {
        ...
        if (resultOfPromoting) {
            SuccessResponse successResponse = successResponseBuilder(
                    LocalDateTime.now(),
                    ResponseMessageConstants.SUCCESSFUL_USER_PROMOTED_MESSAGE,
                    "",
                    true
            );
            return new ResponseEntity<>(this.objectMapper.writeValueAsString(successResponse), HttpStatus.OK);
    ...
    public ResponseEntity demoteUser(@RequestParam(name = "id") String id) throws Exception {
        ...
        if (resultOfDemoting) {
            ...
            SuccessResponse successResponse = successResponseBuilder(
                    LocalDateTime.now(),
                    ResponseMessageConstants.SUCCESSFUL_USER_DEMOTED_MESSAGE,
                    "",
                    true
            );
            return new ResponseEntity<>(this.objectMapper.writeValueAsString(successResponse), HttpStatus.OK);
    ...
    private SuccessResponse successResponseBuilder(LocalDateTime timestamp,
                                                   String message,
                                                   Object payload,
                                                   boolean success) {
